...
class SkullStripper():
    ...
    def deformable_registration(self, atlas_path, anatomy_path):
        ...
        print(self.name)
        aff_reg = utils.get_relative_path(self.name + '_atlas_aff_reg.nii.gz')
        aff_trans = utils.get_relative_path(self.name + '_aff_transformation.txt')
        f3d_cpp = utils.get_relative_path(self.name + '_atlas_f3d_cpp.nii.gz')
